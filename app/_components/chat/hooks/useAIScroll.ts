/**
 * AI 채팅 스크롤 관리를 위한 커스텀 훅
 *
 * 이 훅은 다음과 같은 기능을 제공합니다:
 * 1. 새 AI 메시지가 추가될 때 자동 스크롤
 * 2. 사용자가 스크롤을 임의로 올렸을 때는 자동 스크롤 방지
 * 3. 대기 중일 때는 항상 스크롤 다운
 *
 * @param messages - AI 채팅 메시지 배열
...
interface UseAIScrollOptions {
  smooth?: boolean;
  threshold?: number;
}

export function useAIScroll(
  messages: ChatAIMessage[],
  isWaiting: boolean,
  options: UseAIScrollOptions = {}
) {
